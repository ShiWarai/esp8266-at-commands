...
{
	  uint32_t isrflags   = READ_REG(m_puart->Instance->SR);
	  uint32_t cr1its     = READ_REG(m_puart->Instance->CR1);
    ...
    if (((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
    {
    	 ...
    	 *********************/
		m_puart->Instance->SR;                       /* Read status register */
        unsigned char c = m_puart->Instance->DR;     /* Read data register */
    	...
    	if(tx_buffer.head == tx_buffer.tail)
    	    {
    	      ...
    	      __HAL_UART_DISABLE_IT(m_puart, UART_IT_TXE);
